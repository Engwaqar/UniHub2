...
const ServiceProvider = ({ navigation,route }) => {
  ...
  return (
    <SafeAreaView style={styles.mainContainer} edges={["top", "left", "right"]}>
      <MainHeader navigation={navigation} route={route.params} title={"Book a Cleaner"} />
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            return (
              ...
              >
                <ResponsiveText color={colors.black} size={4}>{item.name}</ResponsiveText>
                <View style={{ flexDirection:'row',justifyContent:'space-between',alignItems:'center',marginTop:8 }} >
                  <ResponsiveText size={3} >★{item.service_rating_count
                      ? parseFloat(
                          item.service_rating_count / item.service_reviews_count
                        ).toFixed(1)
                      : 0}({item.service_reviews_count})</ResponsiveText>
                <Rating
                  type="custom"
