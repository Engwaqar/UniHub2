...
const PaymentConfirmation = ({ navigation, route }) => {
  ...
  const { itemDetail, selectedPakage, date_time, intent, phone, address } =
    route.params;
  const toggel = (url) => {
  ...
  const confirmPay = async (url) => {
    ...
    fetch(url, requestOptions)
      ...
      .catch((error) => {
        ...
        navigation.navigate(routeName.ORDER_PLACED);
        // navigation.dispatch(StackActions.replace(routeName.ORDER_PLACED));

        // navigation.dispatch(
        //   CommonActions.reset({
        //     index: 0,
        //     routes: [{ name:routeName.ORDER_PLACED }],
        //   })
        // );
      });
  };
