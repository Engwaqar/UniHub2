...
import { Alert, FlatList, StyleSheet, TouchableOpacity, View, ScrollView } from "react-native";
...
const BuyerCheckout = ({ navigation ,route}) => {
  ...
  useEffect(() => {
    ...
    // setPhone(MyProfile.phone && MyProfile.phone != "null" ? MyProfile.phone : '');
  }, []);
      ...
      <ScrollView>
        ...
        <View style={{ marginTop: 25 }}>
          <InputText
            ...
            value={phone}
            placeholder={"phone"}
            onChangeText={(text) => text?.length <= 11 && setPhone(text)}
          />
        </View>
        <View style={{ marginTop: 25 }}>
          <InputText
              ...
              return (
                <>
                  ...
                  <ResponsiveText
                      ...
                      size={3}
                    >
                     Sub Total Price: {currency}{item.price*item.pivot?.quantity}
                    </ResponsiveText>
                    <View style={{height:3,width:wp(80),backgroundColor:colors.lightGrey}} />
                </>
              );
            }}
          />
