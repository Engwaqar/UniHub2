...
function* loginUserApi(data, response) {
  ...
  try {
    const response = yield Api.post(urls.NEW_LOGIN, params);
    console.log("response login", response);
    if (response && response.status == 200) {
      if (response.message == "please wait for the admin approvel") {
        _toast(response.message);
      } 
      // else if (
      //   response.message == "User is verified but not complete registeration!"
      // ) {
      //   _toast(response.message);
      //   navigation.replace(routeName.ADD_DETAIL, response.user);
      // } 
      else {
  ...
  try {
    ...
    if (response && response.status == 200) {
      yield put({
        ...
        payload: [
          ...response.event_orders,
          ...response.product_orders,
          ...response.promoted_products,
          ...response.service_orders,
        ],
      });
    } else {
...
export function* getTicketNotificationSaga() {
  yield takeLatest(
    types.GET_TICKET_NOTIFICATION_REQUEST,
    getTicketNotificationApi
  );
}
function* getTicketNotificationApi() {
  try {
    const response = yield Api.get(urls.TICKET_NOTIFICATION);
    console.log("response ticket noti", response);
    if (response && response.status == 200) {
      yield put({
        type: types.GET_TICKET_NOTIFICATION_SUCCESS,
        payload: response.tickets,
      });
    } else {
      yield put({ type: types.GET_TICKET_NOTIFICATION_FAILURE, payload: [] });
    }
  } catch (error) {
    yield put({ type: types.GET_TICKET_NOTIFICATION_FAILURE, error: error });
  }
}
